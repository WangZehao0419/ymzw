  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="520px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="绑定设备ID" prop="equipmentId"><el-input-number v-model="form.equipmentId" :min="1" controls-position="right" style="width:100%" /></el-form-item>
        <el-form-item label="设备名称" prop="equipmentName"><el-input v-model="form.equipmentName" /></el-form-item>
...
<script>
import { listSensor, addSensor, updateSensor, delSensor } from '@/api/business/machine'
export default {
  name: 'BusinessSensor',
  data() { return { loading: false, showSearch: true, total: 0, sensorList: [], open: false, title: '', queryParams: { pageNum: 1, pageSize: 10, sensorCode: '', sensorName: '', sensorStatus: '' }, form: {}, rules: { sensorCode: [{ required: true, message: '传感器编号不能为空', trigger: 'blur' }], sensorName: [{ required: true, message: '传感器名称不能为空', trigger: 'blur' }], sensorUnit: [{ required: true, message: '测量单位不能为空', trigger: 'blur' }], equipmentId: [{ required: true, message: '绑定设备不能为空', trigger: 'change' }] } } },
  ...
  methods: {
    ...
    handleAdd() { this.reset(); this.title = '新增传感器'; this.open = true }, handleUpdate(row) { this.form = { ...row }; this.title = '编辑传感器'; this.open = true },
